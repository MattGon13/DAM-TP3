...
fun getWeatherInfoCodeMap (context: Context): Map <Int , WMO_WeatherInfo>{
    val weatherMap= HashMap <Int , WMO_WeatherInfo >()
    ...
    for(i in codes.indices){
        weatherMap[codes[i].toInt()]= WMO_WeatherInfo(descriptions[i], images[i])
    }
    return weatherMap
}
